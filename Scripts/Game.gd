...
var pre_tribal_torch = preload("res://Scenes/Models/TribalPillarTorchModel.tscn")
...
func send_map_data_to_player(paths_list : Array[PackedVector2Array], bases_list : PackedVector2Array, plants_list : PackedVector2Array):
	for p in players:
		p.update_mini_map_data(paths_list, bases_list, plants_list)
...
func generate_forest() -> void:
	...
	for x in range(int(MAP_SIZE.x/DIVISION_FACTOR + TREE_BORDER_LENGTH)):
		for y in range(int(MAP_SIZE.y/DIVISION_FACTOR + TREE_BORDER_LENGTH)):
			...
			if is_in_base(_new_position) or is_in_path(_new_position) or is_in_mid(_new_position):
				continue
...
const CHANCE_TO_SPAWN_DECORATION := 0.1
func generate_decoration() -> void:
	for path in paths_points_list:
		for p in range(path.size()):
			if randf() > CHANCE_TO_SPAWN_DECORATION:
				continue
			if is_in_base(path[p]) or is_in_mid(path[p]):
				continue
			var _new_tribal_torch = pre_tribal_torch.instantiate()
			...
			_new_tribal_torch.position = Vector3(path[p].x, -0.3, path[p].y) + Vector3(_displace_vec.x, 0.0, _displace_vec.y)
			add_child(_new_tribal_torch)
...
func is_in_mid(pos : Vector2) -> bool:
